...
✦ We obtain $$r(x) = 24 + 19^x + 16x^2 + 24x^3 − 26 − 25^x − 22x^2 $$    
=  $$ 39 + 35x +  35x^2 + 24x^3 $$

✦ So, $$ f(x) × g(x) = r(x) = 39 + 35x + 35x^2 + 24x^3 $$ in $$ R_q $$.

# Representing polynomials as vectors
✦ A polynomial $$ f(x) = a_0 + a_1^x + ⋯ + a_{n−1}x^{n−1} $$ in  $$ R_q = ℤ_q[x]/(x^n + 1)$$ can be represented by its vector of coefficients $$ f = (a_0, a_1, …, a_{n−1}) $$
The vector has length exactly $$n$$ .
✦ Example: Consider $$Rq = ℤ_41[x]/(x^4 + 1) . $$

- ✦ The polynomials $$f(x) = 23 + 11x2 + 7x3 ∈ Rq$$ and $$ g(x) = 40 + 5^x + 16x^2 ∈ R_q $$ can be represented by the vectors f = (23, 0, 11, 7) and g = (40, 5, 16, 0) .
